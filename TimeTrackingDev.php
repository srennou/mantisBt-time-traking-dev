...
*/
require_once('core/timetrackingdev_api.php');
class TimeTrackingDevPlugin extends MantisPlugin
{
	/**
	 * Chart JS
	...
	function register()
	{
		$this->name = 'Time Tracking Dev';
		$this->description = 'Time tracking plugin that Track developers activity';
		...
		$this->author = 'Mohamed Srennou';
		$this->contact = '';
		$this->url = 'https://github.com/mantisbt-plugins/timetrackingdev';
	}
	...
	function resources()
	{
		if (current(explode('/', gpc_get_string('page', ''))) === $this->basename) {
			...
			printf(
				"\t<script src=\"%s\"></script>\n",
				plugin_file('TimeTrackingDev.js')
			);
		}
	}
	/**
	...
	function init()
	{
		...
		plugin_require_api('../MantisGraph/core/graph_api.php');
		plugin_require_api('../MantisGraph/core/Period.php');
		require_api('summary_api.php');
	}


	/**
	 * Show TimeTrackingDev information when viewing bugs.
		...
		<div class="col-md-12 col-xs-12 noprint">
			...
			<div id="timerecord_add" class="widget-box widget-color-blue2 <?php echo $t_block_css ?>">
				...
				<form name="time_tracking" method="post" action="<?php echo plugin_page('add_record') ?>">
					<?php echo form_security_field('plugin_TimeTrackingDev_add_record') ?>
							...
							<div class="table-responsive">
								<table class="width100" cellspacing="1">
									<tr class="row-category">
										<td>
											<div align="center"><b><?php echo plugin_lang_get('expenditure_date'); ?></b></div>
										</td>
										<td>
											<div align="center"><b><?php echo plugin_lang_get('hours'); ?></b></div>
										</td>
										<td>
											<div align="center"><b><?php echo plugin_lang_get('category'); ?></b></div>
										</td>
										<td>
											<div align="center"><b><?php echo plugin_lang_get('information'); ?></b></div>
										</td>
										<td>&nbsp;</td>
									</tr>
									...
									?>
										...
										<tr <?php echo helper_alternate_class() ?>>
											...
											<td>
												<div align="right"><input type="text" name="time_value" value="00:00" size="5" /></div>
											</td>
											<td>
												<div align="center"><select name="time_category"><?php foreach (explode(PHP_EOL, plugin_config_get('categories')) as $t_key) {
																										echo '<option value="' . $t_key . '">' . $t_key . '</option>';
																									} ?></select></div>
											</td>
											<td>
												<div align="center"><input type="text" name="time_info" /></div>
											</td>
											<td>
												<input type="submit" class="btn btn-primary btn-sm btn-white btn-round" value="<?php echo plugin_lang_get('submit') ?>" />
											</td>
										</tr>
									...
									?>
										...
										<tbody>
											<tr>
												...
												<td class="small-caption"><?php echo plugin_TimeTrackingDev_hours_to_hhmm($t_row["hours"]) ?> </td>
												...
												?>
													...
													<td class="small-caption"><a href="<?php echo plugin_page('delete_record') ?>&bug_id=<?php echo $p_bug_id; ?>&delete_id=<?php echo $t_row["id"]; ?><?php echo form_security_param('plugin_TimeTrackingDev_delete_record') ?>"><?php echo plugin_lang_get('delete') ?>
														</a></td>
										...
										<tfoot>
											<tr class="row-category">
												...
												<td class="small-caption">
													<div align="right"><b><?php echo plugin_TimeTrackingDev_hours_to_hhmm($t_row_pull_hours['hours']); ?></b></div>
												</td>
												<td class="small-caption">&nbsp;</td>
												<td class="small-caption">&nbsp;</td>
												<td class="small-caption">&nbsp;</td>
...
<?php
	...
	function schema()
	{
		return array(
			array(
				'CreateTableSQL', array(plugin_table('data'), "
	...
	function timerecord_menu()
	{
		...
		if (
			access_has_bug_level(plugin_config_get('admin_own_threshold'), $bugid)
